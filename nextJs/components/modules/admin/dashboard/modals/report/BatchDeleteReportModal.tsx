...
const BatchDeleteReportModal = ({
  ...
  const {
    ...
    isError,
    error,
  } = trpc.admin.reports.batch.delete.useMutation({
    onSuccess: () => {
      refetchReports();
    },
  });
  ...
  const deleteReportFunc = async () => {
    ...
    toast.promise(
      new Promise(async (resolve, reject) => {
        try {
          await deleteReport({ reportsIds: reportsIds });
          if (isError) {
            throw error;
          }
          resolve(true);
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription className="text-sm sm:text-md">
            ...
            selected reports and remove its data from the servers.
          </DialogDescription>
        </DialogHeader>
...
export default BatchDeleteReportModal;
